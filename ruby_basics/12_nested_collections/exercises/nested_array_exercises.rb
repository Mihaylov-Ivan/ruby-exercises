...
def blank_seating_chart(number_of_rows, seats_per_row)
  ...
  # change with it
  result = Array.new(number_of_rows) {Array.new(seats_per_row)}
end
...
def count_empty_seats(chart)
  ...
  chart.reduce(0){ |total_nils, row|
    total_row_nil_count = row.reduce(0){ |row_nil_count, item|
      item == nil ? row_nil_count + 1 : row_nil_count + 0
    }
    total_nils + total_row_nil_count
  }
end
...
def find_favorite(array_of_hash_objects)
  ...
  array_of_hash_objects.select{ |language|
    language[:is_my_favorite?] == true
  }[0]
end
